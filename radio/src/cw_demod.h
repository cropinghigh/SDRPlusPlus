...
public:
    ...
    void showMenu() {
        ...
        if (ImGui::InputFloatWithScrolling(("##_radio_cw_bw_" + uiPrefix).c_str(), &bw, 1, 100, "%.0f", 0)) {
        ...
        if (ImGui::InputFloatWithScrolling(("##_radio_cw_snap_" + uiPrefix).c_str(), &snapInterval, 1, 100, "%.0f", 0)) {
        ...
        ImGui::SameLine();
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::SliderFloatWithScrolling(("##_radio_cw_squelch_" + uiPrefix).c_str(), &squelchLevel, -100.0f, 0.0f, 5.0f, "%.3fdB")) {
